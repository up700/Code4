...
class Type_Classifier(BertPreTrainedModel):
    def __init__(self, config, type_num_labels, device):
        ...
        #                "science20":torch.tensor([9,10,11,12]).long().to(device),
    ...
    def forward(
        ...
        type_num_labels = self.type_num_labels
        logits_type = self.classifier_type(sequence_output2)
        # if tgt:
    ...
    # def mix_up(self, src_rep, tgt_rep, src_label, tgt_label):
    ...
    #     mix_rep = 0.3*src_exp + 0.7*tgt_exp
    #     mix_label = 0.3*src_onehot + 0.7*tgt_onehot
    #     # print(mix_label)
    #     # exit()

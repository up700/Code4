...
from utils.data_utils_type import load_and_cache_examples, get_labels, get_target_preds, get_sp_loss, get_chunks, tag_to_id
from utils.eval_type import evaluate, evaluate_co
...
def initialize_co(args, tokenizer, t_total, type_num_labels):
    model_class, config_class, _ = MODEL_CLASSES["type"]
    ...
    type_model_bw.to(args.device)

    no_decay = ["bias", "LayerNorm.weight"]

    optimizer_grouped_parameters_type_fw = [
        {
            "params": [p for n, p in type_model_fw.named_parameters() if not any(nd in n for nd in no_decay)],
            "weight_decay": args.weight_decay,
        },
        {"params": [p for n, p in type_model_fw.named_parameters() if any(nd in n for nd in no_decay)], "weight_decay": 0.0},
    ]
    optimizer_type_fw = AdamW(optimizer_grouped_parameters_type_fw, lr=args.learning_rate, \
            eps=args.adam_epsilon, betas=(args.adam_beta1, args.adam_beta2))
    scheduler_type_fw = get_linear_schedule_with_warmup(
        optimizer_type_fw, num_warmup_steps=args.warmup_steps, num_training_steps=t_total
    )

    optimizer_grouped_parameters_type_bw = [
        {
            "params": [p for n, p in type_model_bw.named_parameters() if not any(nd in n for nd in no_decay)],
            "weight_decay": args.weight_decay,
        },
        {"params": [p for n, p in type_model_bw.named_parameters() if any(nd in n for nd in no_decay)], "weight_decay": 0.0},
    ]
    optimizer_type_bw = AdamW(optimizer_grouped_parameters_type_bw, lr=args.learning_rate, \
            eps=args.adam_epsilon, betas=(args.adam_beta1, args.adam_beta2))
    scheduler_type_bw = get_linear_schedule_with_warmup(
        optimizer_type_bw, num_warmup_steps=args.warmup_steps, num_training_steps=t_total
    )

    # Multi-gpu training (should be after apex fp16 initialization)
    if args.n_gpu > 1:
        type_model_fw = torch.nn.DataParallel(type_model_fw)
        type_model_bw = torch.nn.DataParallel(type_model_bw)

    # Distributed training (should be after apex fp16 initialization)
    if args.local_rank != -1:
        type_model_fw = torch.nn.parallel.DistributedDataParallel(
            type_model_fw, device_ids=[args.local_rank], output_device=args.local_rank, find_unused_parameters=True
        )
        type_model_bw = torch.nn.parallel.DistributedDataParallel(
            type_model_bw, device_ids=[args.local_rank], output_device=args.local_rank, find_unused_parameters=True
        )

    type_model_fw.zero_grad()
    type_model_bw.zero_grad()

    return type_model_fw, optimizer_type_fw, scheduler_type_fw, type_model_bw, optimizer_type_bw, scheduler_type_bw

def validation(args, type_model, tokenizer, id_to_label_span, pad_token_label_id, best_dev, best_test, best_dev_bio, best_test_bio,\
         best_dev_tp, best_test_tp, global_step, t_total, epoch):
    best_dev, best_dev_bio, best_dev_tp, is_updated_dev = evaluate(args, type_model, tokenizer, \
    ...
    best_test, best_test_bio, best_test_tp, is_updated_test = evaluate(args, type_model, tokenizer, \
    ...
    return best_dev, best_test, best_dev_bio, best_test_bio, best_dev_tp, best_test_tp, is_updated_dev

def validation_co(args, type_model_fw, type_model_bw, tokenizer, id_to_label_type, pad_token_label_id, best_dev_fw, best_test_fw, best_dev_bw, best_test_bw,\
         global_step, t_total, epoch):
    best_dev_fw, best_dev_bw, is_updated_dev = evaluate_co(args, type_model_fw, type_model_bw, tokenizer, \
        id_to_label_type, pad_token_label_id, best_dev_fw, best_dev_bw, mode="dev", logger=logger, \
        prefix='dev [Step {}/{} | Epoch {}/{}]'.format(global_step, t_total, epoch, args.num_train_epochs), verbose=False)
    best_test_fw, best_test_bw, is_updated_test = evaluate_co(args, type_model_fw, type_model_bw, tokenizer, \
        id_to_label_type, pad_token_label_id, best_test_fw, best_test_bw, mode="test", logger=logger, \
        prefix='test [Step {}/{} | Epoch {}/{}]'.format(global_step, t_total, epoch, args.num_train_epochs), verbose=False)
    ...
    if args.local_rank in [-1, 0] and is_updated_dev[0]:
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        model_to_save = (
            type_model_fw.module if hasattr(type_model_fw, "module") else type_model_fw
        )  # Take care of distributed/parallel training
        model_to_save.save_pretrained(path)
        tokenizer.save_pretrained(path)
    ...
    if args.local_rank in [-1, 0] and is_updated_dev[1]:
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        model_to_save = (
            type_model_bw.module if hasattr(type_model_bw, "module") else type_model_bw
        )  # Take care of distributed/parallel training
        model_to_save.save_pretrained(path)
        tokenizer.save_pretrained(path)
...
def train_source(args, train_dataset, id_to_label_span, id_to_label_type, tokenizer, pad_token_label_id):
    """ Train the model """
    ...
    type_model,\
    optimizer_type, scheduler_type = initialize(args, tokenizer, t_total, span_num_labels, type_num_labels)
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels_type": batch[3], "logits_bio": None, "tgt": False}
            ...
            if (step+1)%args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step%args.logging_steps == 0:
                    ...
                    if args.evaluate_during_training:
                        ...
                        best_dev, best_test, best_dev_bio, best_test_bio, best_dev_tp, best_test_tp, _ = validation(args, type_model, tokenizer, \
...
def co_train(args, train_dataset, id_to_label_type, tokenizer, pad_token_label_id, train_dataset_sl):
    """ Train the model """
    # num_labels = len(labels)
    type_num_labels = len(id_to_label_type)-1
    ...
    sl_model = MODEL_CLASSES["type"][0].from_pretrained(path, type_num_labels=type_num_labels, device=args.device)
    ...
    type_model_fw, optimizer_type_fw, scheduler_type_fw, \
        type_model_bw, optimizer_type_bw, scheduler_type_bw = initialize_co(args, tokenizer, t_total, type_num_labels)
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels_type": batch[3]}
            with torch.no_grad():
                outputs_sl = sl_model(**inputs)
                kl_target = F.softmax(outputs_sl[1], dim=-1).detach()

            outputs_fw = type_model_fw(**inputs)
            outputs_bw = type_model_bw(**inputs)

            kl_fw = F.log_softmax(outputs_fw[1], dim=-1)
            kl_bw = F.log_softmax(outputs_bw[1], dim=-1)

            kl_loss_fw = kl_loss(kl_fw, kl_target)
            kl_loss_bw = kl_loss(kl_bw, kl_target)

            loss += 1.0/(epoch+1)*kl_loss_fw
            loss += 1.0/(epoch+1)*kl_loss_bw

            # Source language
            inputs_sl = {"input_ids": batch_sl[0], "attention_mask": batch_sl[1], "labels_type": batch_sl[3]}
            outputs_fw_sl = type_model_fw(**inputs_sl)
            outputs_bw_sl = type_model_bw(**inputs_sl)
            ...
            if epoch > args.cos_start:
                pseudo_fw, top_prob_fw, threshold_fw = get_target_preds(args.threshold, outputs_fw[1])
                pseudo_bw, top_prob_bw, threshold_bw = get_target_preds(args.threshold, outputs_bw[1])
                ...
                if cos_mask_fw.dim() > 0 and cos_mask_bw.dim() > 0:
                    if cos_mask_fw.numel() > 0 and cos_mask_bw.numel() > 0:
                        ...
                        cos_fw_loss = ce(outputs_fw[1][cos_mask_bw[:cos_mask]], pseudo_bw[cos_mask_bw[:cos_mask]].cuda().detach())
                        cos_bw_loss = ce(outputs_bw[1][cos_mask_fw[:cos_mask]], pseudo_fw[cos_mask_fw[:cos_mask]].cuda().detach())
            ...
            if epoch <= args.penal_start:
                ...
                if sp_mask_fw.dim() > 0 and sp_mask_bw.dim() > 0:
                    if sp_mask_fw.numel() > 0 and sp_mask_bw.numel() > 0:
                        ...
                        sp_fw_loss = get_sp_loss(outputs_fw[1][sp_mask_fw[:sp_mask]], pseudo_fw[sp_mask_fw[:sp_mask]], sp_param_fw)
                        sp_bw_loss = get_sp_loss(outputs_bw[1][sp_mask_bw[:sp_mask]], pseudo_bw[sp_mask_bw[:sp_mask]], sp_param_bw)
                        loss += sp_fw_loss
                        loss += sp_bw_loss
            ...
            if (step+1)%args.gradient_accumulation_steps == 0:
                optimizer_type_fw.step()
                optimizer_type_bw.step()
                scheduler_type_fw.step()  # Update learning rate schedule
                scheduler_type_bw.step()
                type_model_fw.zero_grad()
                type_model_bw.zero_grad()
                global_step += 1
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step%args.logging_steps == 0:
                    ...
                    if args.evaluate_during_training:
                        if epoch <= args.penal_start:
                            try:
                                logger.info("***** training loss : %.4f,  kl_loss_fw: %.4f, kl_loss_bw: %.4f, loss_fw_sl: %.4f, loss_bw_sl: %.4f, sp_fw_loss: %.4f, sp_bw_loss: %.4f *****", \
                                        loss.item(), kl_loss_fw.item(), kl_loss_bw.item(), loss_fw_sl.item(), loss_bw_sl.item(),
                                        sp_fw_loss.item(), sp_bw_loss.item())
                            except:
                                logger.info("***** training loss : %.4f,  kl_loss_fw: %.4f, kl_loss_bw: %.4f, loss_fw_sl: %.4f, loss_bw_sl: %.4f, sp_fw_loss: %.4f, sp_bw_loss: %.4f *****", \
                                        loss.item(), kl_loss_fw.item(), kl_loss_bw.item(), loss_fw_sl.item(), loss_bw_sl.item(),
                                        0, 0)
                        else:
                            logger.info("***** training loss : %.4f,  kl_loss_fw: %.4f, kl_loss_bw: %.4f, loss_fw_sl: %.4f, loss_bw_sl: %.4f, cos_fw_loss: %.4f, cos_bw_loss: %.4f *****", \
                                    loss.item(), kl_loss_fw.item(), kl_loss_bw.item(), loss_fw_sl.item(), loss_bw_sl.item(),
                                    cos_fw_loss.item(), cos_bw_loss.item())
                        best_dev_fw, best_test_fw, best_dev_bw, best_test_bw, _ = validation_co(args, type_model_fw, type_model_bw, tokenizer, \
                            id_to_label_type, pad_token_label_id, best_dev_fw, best_test_fw, best_dev_bw, best_test_bw, \
                            global_step, t_total, epoch)
...
def main():
    ...
    if args.do_train=="true":
        ...
        logger.info("### Start Training Teacher model on Source language in Type Prediction ###")
        best_results = train_source(args, train_dataset_sl,\
            id_to_label_span, id_to_label_type, tokenizer, pad_token_label_id)
        logger.info("### Perform Transfer under SSL ###")
        best_results = co_train(args, train_dataset, id_to_label_type, tokenizer, pad_token_label_id, train_dataset_sl)
        # logger.info(" global_step = %s, average loss = %s", global_step, tr_loss)

    # Testing
    if args.do_test=="true" and args.local_rank in [-1, 0]:
        predict(args, pad_token_label_id, len(id_to_label_type)-1, id_to_label_type, mode="dev")

def predict(args, pad_token_label_id, type_num_labels, id_to_label_type, mode="test"):
    ...
    path_fw = os.path.join(args.output_dir, "checkpoint-best")
    type_model_fw = MODEL_CLASSES["type"][0].from_pretrained(
        path_fw,
        type_num_labels=type_num_labels,
        device=args.device
    )
    ...
    path_bw = os.path.join(args.output_dir, "checkpoint-best")
    type_model_bw = MODEL_CLASSES["type"][0].from_pretrained(
        path_bw,
        type_num_labels=type_num_labels,
        device=args.device
    )
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        if preds_fw is None:
            ...
            h_embed_fw = torch.cat((h_embed_fw, h_e), dim=0)
    ...
    preds_fw_values, preds_fw = torch.max(preds_fw, dim=-1)
    preds_bw_values, preds_bw = torch.max(preds_bw, dim=-1)
    ...
    preds_id_list_fw_values = [[] for _ in range(out_label_ids.shape[0])]

    tp2emb = {"LOC":[], "PER":[], "ORG":[], "MISC":[]}
    ...
    for i in range(out_label_ids.shape[0]):
        for j in range(out_label_ids.shape[1]):
            if out_label_ids[i, j] != pad_token_label_id:
                ...
                preds_id_list_fw_values[i].append(preds_fw_values[i][j])

                ttp = id_to_label_type[out_label_ids[i][j].item()]
                if ttp != "O":
                    tp2emb[ttp].append(h_embed_fw[i][j].cpu().numpy().tolist())
    
    # if flag:
    with open("log_dev_embedding_de.txt", "w", encoding="utf-8") as f:
        for tt in tp2emb:
            for ll in tp2emb[tt][:600]:
                ll = [str(nu) for nu in ll]
                f.write("de "+tt+" "+" ".join(ll)+"\n")
    # exit()
    ...
    types = tag_to_id(args.data_dir, args.dataset)["type"]
    idx_to_type = {idx: t for t, idx in types.items()}

    for txt, ground_truth_id_type, predicted_id_type_fw, predicted_id_type_bw, ground_truth_id_bio, predicted_id_type_fw_v in zip(texts, out_id_list, \
                                                            preds_id_list_fw, preds_id_list_bw, span_id_list, preds_id_list_fw_values):
        lab_chunks, lab_chunks_bio = get_chunks(ground_truth_id_type, ground_truth_id_bio, tag_to_id(args.data_dir, args.dataset))

        txt["net_gold"] = ["O"]*len(txt["tags_net"])
        for ii in lab_chunks:
            txt["net_gold"][ii[1]:ii[2]] = [ii[0]]*(ii[2]-ii[1])
        ...
        txt["net_pred_fw"] = ["O"]*len(txt["tags_net"])
        for idx, (ind, v) in enumerate(zip(predicted_id_type_fw, predicted_id_type_fw_v)):
            txt["net_pred_fw"][idx] = idx_to_type[ind.item()]
        ...
        txt["net_pred_bw"] = ["O"]*len(txt["tags_net"])
        for ii in lab_pred_chunks_tp_bw:
            txt["net_pred_bw"][ii[1]:ii[2]] = [ii[0]]*(ii[2]-ii[1])
    ...
    with open(os.path.join(args.output_dir, args.dataset+"_"+mode+"_pred_types.json"), "w", encoding="utf-8") as f:

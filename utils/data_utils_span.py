...
def read_examples_from_file(args, data_dir, mode):
    ...
    if mode == "train":
        ...
        # file_path = os.path.join(data_dir, "inter_{}_{}.json".format(args.dataset, mode))
        # with open(file_path, "r") as f:
        #     data = json.load(f)
        #     for item in data:
        #         words = item["str_words"]
        ...
        #         labels_esi = item["tags_esi"]
        #         labels_net = item["tags_net"]
        #         examples_inter.append(InputExample(guid="%s-%d".format(mode, guid_index), words=words, labels=labels_ner, spans=labels_esi, types=labels_net))
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for word, label in zip(example.words, example.spans):
            # print(word, label)
            # span_label = tag_to_id["span"][span_label]
            # type_label = tag_to_id["type"][type_label]
            # label = tag_to_id["ner"][label]
            label = tag_to_id["span"][label]
        ...
        features.append(
            InputFeatures(input_ids=input_ids, input_mask=input_mask, segment_ids=None, \
                label_ids=label_ids, full_label_ids=None, span_label_ids=None, type_label_ids=None, label_mask=label_mask)
        )
...
def load_and_cache_examples(args, tokenizer, pad_token_label_id, mode):
    ...
    dataset_meta = TensorDataset(all_input_ids, all_input_mask, all_label_ids, all_label_mask, all_ids) # 添加 segment
    ...
    if mode == "train":
        ...
        all_label_ids_src = torch.tensor([f.label_ids for f in features_src], dtype=torch.long)
        ...
        dataset = TensorDataset(all_input_ids_src, all_input_mask_src, all_label_ids_src, all_label_mask_src, all_ids_src) # 添加segment
...
def get_chunk_type(tok, idx_to_tag):
    ...
    """
    tag_name = idx_to_tag[tok.item()]
    tag_class = tag_name.split('-')[0]
    tag_type = tag_name.split('-')[-1]
    return tag_class, tag_type

def get_chunks_bio(seq_bio, tags):
...
def get_chunks(seq, tags):
    ...
    """
    default = tags["ner"]["O"]
    idx_to_tag = {idx: tag for tag, idx in tags["ner"].items()}
    chunks = []
